...
def sac(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def test_agent():
        ...
        for j in range(num_test_episodes):
            ...
            while not (d or (ep_len == max_ep_len)):
                ...
                a = get_action(o, True)
                o2, r, d, _ = test_env.step(a.cpu().numpy())
                ep_ret += r
                ep_len += 1
            ...
            wandb.log({"TestEpRet": ep_ret, "EpLen": ep_len}, step=global_t + j)  
        global_t += num_test_episodes  # Increment global_t by number of test episodes
    ...
    for t in range(total_steps):
        ...
        d = torch.tensor(False if ep_len == max_ep_len else d.item(), dtype=torch.float32, device=device)
        ...
        if d.item() or (ep_len == max_ep_len):
            # Log epoch return
